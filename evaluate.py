...
#    target_abs = T.complex_abs(T.ifft2(kspace_t))
...
def get_recons_target(recons_file,target_file):
   ...
   recons = np.load(recons_file)
   recons = torch.from_numpy(recons).squeeze(1).float().numpy()

   num_slices = recons.shape[0]
   

   for i in range(num_slices):
        recons[i] = recons[i] / target_abs[i].max()
        target_abs[i] = target_abs[i] / target_abs[i].max()
        
   
   return recons , target_abs

...
NUM_EPOCHS=100
ACC_FACTOR=8
EXP_DIR='/media/student1/NewVolume/MR_Reconstruction/experiments/singlechannel_calgary/acc_'${ACC_FACTOR}'x/dualencoder/scratch/'${N0_OF_VOLUMES}'_volumes/'${LEARNING_RATE}
...
python train_wnet_pretext.py --batch-size ${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --device ${DEVICE} --exp-dir ${EXP_DIR} --train-path ${TRAIN_PATH} --validation-path ${VALIDATION_PATH} --acceleration_factor ${ACC_FACTOR} --dataset_type ${DATASET_TYPE}  --sample ${N0_OF_VOLUMES} --lr ${LEARNING_RATE} 
PRETRAINING_OF_WNET
...
python train_wnet_finetune.py --batch-size ${BATCH_SIZE} --num-epochs ${NUM_EPOCHS} --device ${DEVICE} --exp-dir ${EXP_DIR} --train-path ${TRAIN_PATH} --validation-path ${VALIDATION_PATH} --acceleration_factor ${ACC_FACTOR} --dataset_type ${DATASET_TYPE}  --sample ${N0_OF_VOLUMES} --lr ${LEARNING_RATE} --pretext ${PRETEXT_MODEL} 
FINETUNING_OF_DUALENCODER
